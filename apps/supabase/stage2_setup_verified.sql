...
-- ────────────────────────────────────────────────────────────────
select 'surveys table exists'        as check,
       count(*)::text                as result
from information_schema.tables
where table_schema = 'public' and table_name = 'surveys'
union all
select 'demo response table exists',
       count(*)::text
from information_schema.tables
where table_schema = 'public' and table_name = 'survey_engine_demo'
union all
select 'demo survey registered',
       count(*)::text
from public.surveys where slug = 'engine-demo';
