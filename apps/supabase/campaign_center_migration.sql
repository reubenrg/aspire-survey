...
create table if not exists public.survey_campaigns (
  ...
  -- Snapshot of surveys.privacy_mode at creation: a later privacy-mode change
  -- on the survey must not retroactively relabel a campaign already launched
  -- under the mode that was actually in effect when it went out.
  privacy_mode public.survey_privacy_mode not null,
  status public.campaign_status not null default 'DRAFT',
  sender_name text not null default 'Aspire Surveys',
  -- Sandbox sender until a verified custom domain is configured - see the
  -- production_email_domain_verified platform_settings gate below.
  ...
  preview_text text,
  -- Plain text, blank-line-separated paragraphs plus {{merge}} tokens. Never
  -- raw HTML: the send path escapes this and substitutes tokens into a fixed,
  -- Aspire-branded HTML template server-side, so there is no HTML injection
...
create table if not exists public.survey_campaign_recipients (
  ...
  -- Reuses an existing survey_invitations row. No competing identity/token
  -- model: this is the only link between a campaign and a person.
...
-- Append-only event log per recipient send attempt - a retry logs a new
-- event rather than overwriting history, so delivery_status on the recipient
-- row (the current/derived state) can be recomputed or audited later.
...
-- Test sends are structurally isolated: no foreign key to employees or to
-- survey_invitations exists anywhere on this table, so a test send cannot
-- reach - and therefore cannot mutate - any real employee's invitation state.
...
-- A configuration row, not a code constant: flipping this later (once a
-- custom domain is verified in Resend) should require an update to this
-- setting only, never a rebuild of the campaign architecture. Defaults to
-- false - real bulk employee sending stays disabled until a founder
-- explicitly turns this on after verifying the domain.
...
-- No direct insert/update grant on recipients: rows are only created by
-- build_campaign_recipients() below (editor-gated) and mutated only by the
-- send Edge Function's service-role connection, which bypasses RLS entirely
-- and is the sole path that ever calls Resend. Analysts and viewers can read.
...
-- No direct insert grant: rows are created only by request_test_send() below,
-- and updated only by the Edge Function's service-role connection.
...
-- The real STARTED implementation. Called by the respondent-facing engine
-- the moment someone leaves the welcome screen (before this, STARTED was
-- defined in the enum and read by audience_summary()/REMINDABLE_STATUSES,
-- but nothing ever wrote it - the status silently never fired). Anonymous
-- (non-invitation) surveys have no invitation to update, so this is a no-op
-- for them by construction (token_hash simply won't match any row).
...
-- Ensures every given employee has an invitation for this campaign's survey
-- (reusing issue_invitations - idempotent, raw tokens discarded, exactly the
-- convention audienceStore.ts's buildAudience() already follows), then links
-- each resulting invitation to the campaign. Existing campaign_recipients
-- rows are left untouched, so calling this again after the audience grows
-- only adds the new people.
...
-- Monitor-screen aggregate: delivery state (this campaign's own concern) kept
-- deliberately separate from survey participation state (the invitation's
-- own concern, unaffected by which campaign - if any - sent the link).
...
-- Generates a fresh test token and records the test send request. The raw
-- token is returned once, exactly like regenerate_invitation already does
-- for real invitations, and is never stored - only its hash is.
...
-- Mirrors resolve_invitation, but resolves a test token instead. Never reads
-- or writes anything on survey_invitations or employees.
...
-- Mirrors submit_invited_response, but deliberately discards the payload
-- entirely: a test send exists only to prove rendering and the completion
-- screen, never to produce data. No per-survey response table is ever
-- touched, so a test can never appear in real analytics.
...
-- Service-role-only variant of regenerate_invitation, used exclusively by the
-- send-campaign Edge Function to resolve a fresh raw token for each recipient
-- at send time (build_campaign_recipients discards tokens the moment
-- invitations are created, matching the existing convention, so a later send
-- pass must regenerate one). Deliberately has NO has_survey_role check: a
-- service-role connection carries no user JWT, so that check would always
-- fail for it. Safety instead comes from the grant below - only the
-- service_role Postgres role can ever call this, never anon or authenticated,
-- so a browser client cannot reach it under any circumstances.
...
-- Explicit lifecycle transitions. Each is a single, auditable, editor-gated
-- write - the Edge Function itself only ever moves a campaign into SENDING
-- and then to SENT/PARTIALLY_FAILED once a send pass finishes.
...
-- Reminder eligibility, computed the same way REMINDABLE_STATUSES already
-- does client-side (labels.ts) - kept here too as a server-side source of
-- truth the send Edge Function can use directly, so eligibility can never
-- drift between what the UI shows and what actually gets emailed.
