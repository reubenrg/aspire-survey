-- HIGH fix: the open /s/:slug path had no CLOSED-survey enforcement anywhere.
-- The anon SELECT policy on surveys only checked published=true (never
-- closed_at), and every response table's own INSERT policy - generated by
-- ensure_survey_response_table()/generateSql.ts - was a static `with check
-- (true)`, so submitResponse() (a direct client insert, no security-definer
-- function in the loop for the open path) could still write a row after an
-- admin closed the survey. The invitation path already got this right:
-- submit_invited_response() explicitly checks s.closed_at is not null.
--
-- Fixes the real boundary (RLS on the response table) so this holds even for
-- a request that bypasses the UI entirely, and updates the function that
-- creates that policy going forward so every future-provisioned table gets
-- it automatically.
...
begin
  ...
  -- Insert is now gated on the survey's own live state, not a bare `true`:
  -- a closed or unpublished survey rejects the insert at the RLS layer
  -- itself, so this holds even for a request that bypasses the app's UI.
...
-- Re-apply the new INSERT policy to every already-provisioned response
-- table, so existing surveys get the fix immediately rather than only on
-- their next publish. (create_response_policy_only loop - no column/schema
-- changes, purely re-running the policy-creation portion for each table.)
--
-- NOTE: this loop's has_survey_role() gate turned out to be a no-op in a
-- migration's privileged execution context (auth.jwt() is empty there), so
-- every table was silently skipped - see the very next migration
-- (retrofit_closed_check_on_existing_response_tables), which removes the
-- gate and actually performs the backfill. Kept here byte-exact to what was
-- really executed, for an accurate history.
do $$
declare r record;
begin
  for r in select id, organization_id, table_name from public.surveys
           where table_name is not null and to_regclass('public.' || table_name) is not null
  loop
    if not public.has_survey_role(r.organization_id, 'editor') then continue; end if;
