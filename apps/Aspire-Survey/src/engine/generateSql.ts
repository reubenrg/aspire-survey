...
export function generateCreateTableSql(def: SurveyDefinition): string {
  ...
  const lines = [
    ...
    ``,
    ``,
    `-- Analysts and above may read responses in full. anon still cannot: there`,
    `-- is no select policy for it, so a respondent cannot read anybody's`,
    `-- answers including their own.`,
