...
1. **Domain / Vercel project.** `aspiresurveys.vercel.app` is served by a Vercel project outside the team the build tooling could see. Confirm its production branch is `main` and that `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` are set for Production. The old project (`s2m-aspire-survey`, team `aspiresurvey`) also builds `main`, so it now serves the admin at the old domain; delete that project or its domains if you do not want two public copies. Old deployment snapshots (including the S2M survey) are SSO-protected, not public, but can be deleted in the Vercel dashboard.
