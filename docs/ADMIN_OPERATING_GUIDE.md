...
## Scoring, webhooks and drafting a survey

- **Scoring (quiz):** *Survey settings → Scoring*. Switch it on, then open any choice, yes/no, rating, NPS, slider or number question and set its **Points**. Optionally add **result bands** (a label and message from a minimum score) and show the respondent their score; `{{score}}`, `{{maxscore}}` and `{{result}}` work in the thank-you message. The total is stored in a `quiz_score` column and appears in Analytics. It is computed in the respondent's browser, so it suits quizzes and self-assessments but is not tamper-proof.
- **Webhooks:** *Survey settings → Send responses to another app*. Paste a Zapier "Catch Hook", Make, Teams or Slack incoming-webhook address (or your own https endpoint), choose Full JSON or Chat message, and use **Send test**. The signing secret is shown once when you add it; every request carries `X-Aspire-Signature: sha256=<HMAC-SHA256 of the raw body>`. Respondent names, employee IDs and departments are never sent. The survey must be published first. Up to five per survey.
- **Start a survey by pasting or with AI:** on *New survey* choose **Paste questions** (turns a list from a document into a survey) or **Draft with AI** (describe the survey; you review the draft before anything is created). AI never drafts file uploads, signatures or questions that collect names, emails or phone numbers, and everything it returns goes through the same checks as a hand-built survey. Drafting needs the `ANTHROPIC_API_KEY` secret on the `generate-survey` Edge Function; until an administrator adds it, the screen says so.
